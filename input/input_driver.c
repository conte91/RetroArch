...
#define INPUT_REMOTE_KEY_PRESSED(input_st, key, port) \
   (input_st->remote_st_ptr.buttons[(port)] & (UINT64_C(1) << (key)))
...
#define CHECK_INPUT_DRIVER_BLOCK_HOTKEY(normal_bind, autoconf_bind)                     \
   ((((normal_bind)->key != RETROK_UNKNOWN) || ((normal_bind)->mbutton != NO_BTN) ||    \
     ((normal_bind)->joykey != NO_BTN) || ((normal_bind)->joyaxis != AXIS_NONE) ||      \
     ((autoconf_bind)->key != RETROK_UNKNOWN) || ((autoconf_bind)->joykey != NO_BTN) || \
...
static int16_t input_null_input_state(void *data, const input_device_driver_t *joypad,
                                      const input_device_driver_t *sec_joypad,
                                      rarch_joypad_info_t *joypad_info,
                                      const retro_keybind_set *retro_keybinds,
                                      bool keyboard_mapping_blocked, unsigned port, unsigned device,
                                      unsigned index, unsigned id)
{
   return 0;
}
static void input_null_free(void *data) {}
static bool input_null_set_sensor_state(void *data, unsigned port, enum retro_sensor_action action,
                                        unsigned rate)
{
   return false;
}
...
static input_driver_t input_null = {input_null_init,
                                    input_null_poll,
...
#ifdef HAVE_HID
static bool null_hid_joypad_query(void *data, unsigned pad) { return pad < MAX_USERS; }
static const char *null_hid_joypad_name(void *data, unsigned pad) { return NULL; }
static void null_hid_joypad_get_buttons(void *data, unsigned port, input_bits_t *state)
{
   BIT256_CLEAR_ALL_PTR(state);
}
static int16_t null_hid_joypad_button(void *data, unsigned port, uint16_t joykey) { return 0; }
static bool null_hid_joypad_rumble(void *data, unsigned pad, enum retro_rumble_effect effect,
                                   uint16_t strength)
{
   return false;
}
static int16_t null_hid_joypad_axis(void *data, unsigned port, uint32_t joyaxis) { return 0; }
static void *null_hid_init(void) { return (void *) -1; }
static void null_hid_free(const void *data) {}
static void null_hid_poll(void *data) {}
static int16_t null_hid_joypad_state(void *data, rarch_joypad_info_t *joypad_info,
...
input_driver_state_t *input_state_get_ptr(void) { return &input_driver_st; }
...
static const input_device_driver_t *input_joypad_init_first(void *data)
{
   ...
   {
      if (joypad_drivers[i] && joypad_drivers[i]->init)
      {
...
bool input_driver_set_rumble(unsigned port, unsigned joy_idx, enum retro_rumble_effect effect,
                             uint16_t strength)
{
...
bool input_driver_set_rumble_gain(unsigned gain, unsigned input_max_users)
{
   int i;

   if (input_driver_st.primary_joypad && input_driver_st.primary_joypad->set_rumble_gain)
   {
...
bool input_driver_set_sensor(unsigned port, bool sensors_enable, enum retro_sensor_action action,
                             unsigned rate)
{
   ...
   if (!sensors_enable &&
       ((action == RETRO_SENSOR_ACCELEROMETER_ENABLE) ||
        (action == RETRO_SENSOR_GYROSCOPE_ENABLE) || (action == RETRO_SENSOR_ILLUMINANCE_ENABLE)))
      return false;
   if ((current_driver = input_driver_st.current_driver) && current_driver->set_sensor_state)
   {
      void *current_data = input_driver_st.current_data;
      return current_driver->set_sensor_state(current_data, port, action, rate);
   }
...
float input_driver_get_sensor(unsigned port, bool sensors_enable, unsigned id)
{
   if (input_driver_st.current_data)
   {
...
const input_device_driver_t *input_joypad_init_driver(const char *ident, void *data)
{
   if (ident && *ident)
   {
      ...
      for (i = 0; joypad_drivers[i]; i++)
      {
         if (string_is_equal(ident, joypad_drivers[i]->ident) && joypad_drivers[i]->init)
         {
            void *ptr = joypad_drivers[i]->init(data);
            if (ptr)
            {
...
bool input_driver_button_combo(unsigned mode, retro_time_t current_time, input_bits_t *p_input)
{
   switch (mode)
   {
...
static int16_t input_state_wrap(input_driver_t *current_input, void *data,
                                const input_device_driver_t *joypad,
                                const input_device_driver_t *sec_joypad,
                                rarch_joypad_info_t *joypad_info, const retro_keybind_set *binds,
                                bool keyboard_mapping_blocked, unsigned _port, unsigned device,
                                unsigned idx, unsigned id)
{
   ...
   if (device == RETRO_DEVICE_JOYPAD)
   {
      ...
      else
      {
         ...
         if (binds[_port][id].valid)
         {
            ...
            const uint64_t joyaxis = (bind_joyaxis != AXIS_NONE) ? bind_joyaxis : autobind_joyaxis;

            if (joypad)
            {
               if ((uint16_t) joykey != NO_BTN && joypad->button(port, (uint16_t) joykey))
                  return 1;
               if (joyaxis != AXIS_NONE &&
                   ((float) abs(joypad->axis(port, (uint32_t) joyaxis)) / 0x8000) > axis_threshold)
                  return 1;
            }
            if (sec_joypad)
            {
               if ((uint16_t) joykey != NO_BTN && sec_joypad->button(port, (uint16_t) joykey))
                  return 1;
               if (joyaxis != AXIS_NONE &&
                   ((float) abs(sec_joypad->axis(port, (uint32_t) joyaxis)) / 0x8000) >
                      axis_threshold)
                  return 1;
            }
         }
   ...
   if (current_input && current_input->input_state)
      ret |= current_input->input_state(data, joypad, sec_joypad, joypad_info, binds,
                                        keyboard_mapping_blocked, _port, device, idx, id);
   return ret;
}

static int16_t input_joypad_axis(float input_analog_deadzone, float input_analog_sensitivity,
                                 const input_device_driver_t *drv, unsigned port, uint32_t joyaxis,
                                 float normal_mag)
{
   ...
   if (input_analog_deadzone)
   {
      ...
      val = val * MAX(1.0f, (1.0f / normal_mag)) *
            MIN(1.0f, ((normal_mag - input_analog_deadzone) / (1.0f - input_analog_deadzone)));
   }
 ...
 **/
static int16_t input_joypad_analog_button(float input_analog_deadzone,
                                          float input_analog_sensitivity,
                                          const input_device_driver_t *drv,
                                          rarch_joypad_info_t *joypad_info, unsigned ident,
                                          const struct retro_keybind *bind)
{
   int16_t res = 0;
   float normal_mag = 0.0f;
   uint32_t axis =
      (bind->joyaxis == AXIS_NONE) ? joypad_info->auto_binds[ident].joyaxis : bind->joyaxis;
   ...
   if ((res = abs(input_joypad_axis(input_analog_deadzone, input_analog_sensitivity, drv,
                                    joypad_info->joy_idx, axis, normal_mag))) == 0)
   {
      uint16_t key =
         (bind->joykey == NO_BTN) ? joypad_info->auto_binds[ident].joykey : bind->joykey;
...
static int16_t input_joypad_analog_axis(unsigned input_analog_dpad_mode,
                                        float input_analog_deadzone, float input_analog_sensitivity,
                                        const input_device_driver_t *drv,
                                        rarch_joypad_info_t *joypad_info, unsigned idx,
                                        unsigned ident, const struct retro_keybind *binds)
{
   int16_t res = 0;
...

   {
      ...
      res = abs(input_joypad_axis(input_analog_deadzone, input_analog_sensitivity, drv,
                                  joypad_info->joy_idx, axis_plus, normal_mag));
      res -= abs(input_joypad_axis(input_analog_deadzone, input_analog_sensitivity, drv,
                                   joypad_info->joy_idx, axis_minus, normal_mag));
   }
   ...
   if (res == 0)
   {
      uint16_t key_minus = (bind_minus->joykey == NO_BTN)
                              ...
                              : bind_minus->joykey;
      uint16_t key_plus = (bind_plus->joykey == NO_BTN) ? joypad_info->auto_binds[ident_plus].joykey
                                                        : bind_plus->joykey;
...
void input_keyboard_line_append(struct input_keyboard_line *keyboard_line, const char *word,
                                size_t len)
{
   size_t i;
   char *newbuf = (char *) realloc(keyboard_line->buffer, keyboard_line->size + len * 2);

   if (!newbuf)
      return;

   memmove(newbuf + keyboard_line->ptr + len, newbuf + keyboard_line->ptr,
...
const char **input_keyboard_start_line(void *userdata, struct input_keyboard_line *keyboard_line,
...
static bool input_remote_init_network(input_remote_t *handle, uint16_t port, unsigned user)
{
   int fd;
...
static input_remote_t *input_remote_new(settings_t *settings, uint16_t port, unsigned max_users)
{
   int user;
...
static void input_remote_parse_packet(input_remote_state_t *input_state, struct remote_message *msg,
                                      unsigned user)
{
   /* Parse message */
   switch (msg->device)
...
input_remote_t *input_driver_init_remote(settings_t *settings, unsigned num_active_users)
{
   unsigned network_remote_base_port = settings->uints.network_remote_base_port;
   return input_remote_new(settings, network_remote_base_port, num_active_users);
}
#endif
 ...
 */
static bool input_overlay_add_inputs_inner(overlay_desc_t *desc, input_overlay_state_t *ol_state,
                                           unsigned port)
{
   switch (desc->type)
   {
         ...
         for (i = 0; i < CUSTOM_BINDS_U32_COUNT; ++i)
         {
            ...
            while (bank_mask)
            {
               ...
               if (bank_mask & 1)
               {
                  ...
                  if (ol_state ? !BIT256_GET(ol_state->buttons, id)
      ...
      case OVERLAY_TYPE_ANALOG_RIGHT:
         if (ol_state)
         {
            ...
            desc->updated |= (ol_state->analog[index_offset] | ol_state->analog[index_offset + 1]);
         }
         else
         {
            unsigned index = (desc->type == OVERLAY_TYPE_ANALOG_RIGHT)
                                ...
                                : RETRO_DEVICE_INDEX_ANALOG_LEFT;
            float analog_x =
               input_state_internal(port, RETRO_DEVICE_ANALOG, index, RETRO_DEVICE_ID_ANALOG_X);
            float analog_y =
               input_state_internal(port, RETRO_DEVICE_ANALOG, index, RETRO_DEVICE_ID_ANALOG_Y);
      ...
      case OVERLAY_TYPE_KEYBOARD:
         if (ol_state ? OVERLAY_GET_KEY(ol_state, desc->retro_key_idx)
...
static bool input_overlay_add_inputs(input_overlay_t *ol, bool show_touched, unsigned port)
{
   size_t i;
   ...
   for (i = 0; i < ol->active->size; i++)
   {
      ...
      button_pressed |= input_overlay_add_inputs_inner(desc, show_touched ? ol_state : NULL, port);
   }

   return button_pressed;
}

static void input_overlay_get_eightway_slope_limits(const unsigned diagonal_sensitivity,
...
void input_overlay_set_eightway_diagonal_sensitivity(void)
{
   ...
   input_overlay_get_eightway_slope_limits(settings->uints.input_overlay_dpad_diagonal_sensitivity,
                                           &input_st->overlay_eightway_dpad_slopes[0],
                                           &input_st->overlay_eightway_dpad_slopes[1]);

   input_overlay_get_eightway_slope_limits(settings->uints.input_overlay_abxy_diagonal_sensitivity,
 ...
 **/
static INLINE void input_overlay_get_eightway_state(const struct overlay_desc *desc,
                                                    input_bits_t *out, float x_dist, float y_dist)
{
 ...
 **/
static void input_overlay_poll(input_overlay_t *ol, input_overlay_state_t *out, unsigned ptr_idx,
                               int16_t norm_x, int16_t norm_y, float touch_scale)
{
   ...
   for (i = 0; i < ol->active->size; i++)
   {
      ...
      {
         case OVERLAY_TYPE_BUTTONS:
            bits_or_bits(out->buttons.data, desc->button_mask.data,
         ...
         case OVERLAY_TYPE_DPAD_AREA:
         case OVERLAY_TYPE_ABXY_AREA:
            input_overlay_get_eightway_state(desc, &out->buttons, x_dist, y_dist);
            break;
         case OVERLAY_TYPE_ANALOG_RIGHT:
            base = 2;
            /* fall-through */
...
static void input_overlay_update_desc_geom(input_overlay_t *ol, struct overlay_desc *desc)
{
   ...
   if (ol->iface->vertex_geom)
      ol->iface->vertex_geom(ol->iface_data, desc->image_index, desc->mod_x + desc->delta_x,
                             desc->mod_y + desc->delta_y, desc->mod_w, desc->mod_h);
 ...
 * and alpha mods.
 **/
static void input_overlay_post_poll(enum overlay_visibility *visibility, input_overlay_t *ol,
      ...
      if (desc->updated != 0)
      {
         ...
         if (show_input && desc->image.pixels)
         {
            if (ol->iface->set_alpha)
               ol->iface->set_alpha(ol->iface_data, desc->image_index, desc->alpha_mod * opacity);
         }
      }
...
{
   desc->x_hitbox = ((desc->x_shift + desc->range_x * desc->reach_right) +
                     (desc->x_shift - desc->range_x * desc->reach_left)) /
                    2.0f;

   desc->y_hitbox = ((desc->y_shift + desc->range_y * desc->reach_down) +
                     (desc->y_shift - desc->range_y * desc->reach_up)) /
                    2.0f;

   desc->range_x_hitbox =
      (desc->range_x * desc->reach_right + desc->range_x * desc->reach_left) / 2.0f;

   desc->range_y_hitbox =
      (desc->range_y * desc->reach_down + desc->range_y * desc->reach_up) / 2.0f;
...
static void input_overlay_scale(struct overlay *ol, const overlay_layout_t *layout)
{
   ...
   ol->mod_x = (ol->center_x + (ol->x - ol->center_x) * layout->x_scale) + layout->x_offset;
   ol->mod_y = (ol->center_y + (ol->y - ol->center_y) * layout->y_scale) + layout->y_offset;
...
static void input_overlay_parse_layout(const struct overlay *ol,
                                       const overlay_layout_desc_t *layout_desc,
                                       float display_aspect_ratio, overlay_layout_t *overlay_layout)
{
   /* Set default values */
   ...
   if (layout_desc->auto_scale)
   {
       ...
       * can do nothing */
      if ((ol->flags & OVERLAY_BLOCK_SCALE) || (ol->aspect_ratio <= 0.0f) ||
      ...
      {
         overlay_layout->x_scale = ol->aspect_ratio / display_aspect_ratio;
      ...
      {
         overlay_layout->y_scale = display_aspect_ratio / ol->aspect_ratio;
...
static void input_overlay_set_vertex_geom(input_overlay_t *ol)
{
   ...
   if (ol->active->image.pixels)
      ol->iface->vertex_geom(ol->iface_data, 0, ol->active->mod_x, ol->active->mod_y,
   ...
   for (i = 0; i < ol->active->size; i++)
   {
      ...
      if (desc->image.pixels)
         ol->iface->vertex_geom(ol->iface_data, desc->image_index, desc->mod_x, desc->mod_y,
 ...
 * offset factors.
 **/
void input_overlay_set_scale_factor(input_overlay_t *ol, const overlay_layout_desc_t *layout_desc,
                                    unsigned video_driver_width, unsigned video_driver_height)
{
   size_t i;
   ...
   if (video_driver_height > 0)
      display_aspect_ratio = (float) video_driver_width / (float) video_driver_height;
   ...
   for (i = 0; i < ol->size; i++)
   {
      ...
      input_overlay_parse_layout(current_overlay, layout_desc, display_aspect_ratio,
                                 &overlay_layout);
...
void input_overlay_load_active(enum overlay_visibility *visibility, input_overlay_t *ol,
                               float opacity)
{
   if (ol->iface->load)
   ...
   if (ol->iface->full_screen)
      ol->iface->full_screen(ol->iface_data, (ol->active->flags & OVERLAY_FULL_SCREEN));
}
 ...
 * clear certain state.
 **/
static void input_overlay_poll_clear(enum overlay_visibility *visibility, input_overlay_t *ol,
                                     float opacity)
{
   size_t i;
...
static enum overlay_visibility input_overlay_get_visibility(enum overlay_visibility *visibility,
                                                            int overlay_idx)
{
   if (!visibility)
...
void input_overlay_set_alpha_mod(enum overlay_visibility *visibility, input_overlay_t *ol,
                                 float mod)
{
   unsigned i;
   ...
   {
      if (input_overlay_get_visibility(visibility, i) == OVERLAY_VISIBILITY_HIDDEN)
 ...
 **/
static void input_poll_overlay(bool keyboard_mapping_blocked, settings_t *settings, void *ol_data,
                               enum overlay_visibility *overlay_visibility, float opacity,
                               unsigned analog_dpad_mode, float axis_threshold)
{
   ...
   enum overlay_show_input_type input_overlay_show_inputs =
      (enum overlay_show_input_type) settings->uints.input_overlay_show_inputs;
   ...
   memcpy(&old_ol_state, ol_state, sizeof(old_ol_state));
   memset(ol_state, 0, sizeof(*ol_state));

   if (current_input->input_state)
   {
      rarch_joypad_info_t joypad_info;
      unsigned device = (ol->active->flags & OVERLAY_FULL_SCREEN) ? RARCH_DEVICE_POINTER_SCREEN
      ...
      for (i = 0; current_input->input_state(input_data, joypad, sec_joypad, &joypad_info, NULL,
                                             keyboard_mapping_blocked, 0, device, i,
                                             RETRO_DEVICE_ID_POINTER_PRESSED);
           i++)
      {
         input_overlay_state_t polled_data;
         int16_t x = current_input->input_state(input_data, joypad, sec_joypad, &joypad_info, NULL,
                                                keyboard_mapping_blocked, 0, device, i,
                                                RETRO_DEVICE_ID_POINTER_X);
         int16_t y = current_input->input_state(input_data, joypad, sec_joypad, &joypad_info, NULL,
                                                keyboard_mapping_blocked, 0, device, i,
         ...
         bits_or_bits(ol_state->buttons.data, polled_data.buttons.data,
   ...
   button_pressed =
      input_overlay_add_inputs(ol, (input_overlay_show_inputs == OVERLAY_SHOW_INPUT_TOUCHED),
   ...
   if (button_pressed || touch_count)
      input_overlay_post_poll(overlay_visibility, ol, button_pressed, opacity);
   else
   ...
   if (current_input->keypress_vibrate && settings->bools.vibrate_on_keypress && touch_count &&
       touch_count >= old_touch_count && !(ol->flags & INPUT_OVERLAY_BLOCKED))
   {
      if (osk_state_changed || bits_any_different(ol_state->buttons.data, old_ol_state.buttons.data,
...
void input_config_get_bind_string(void *settings_data, char *buf, const struct retro_keybind *bind,
                                  const struct retro_keybind *auto_bind, size_t size)
{
   ...
   *buf = '\0';
   ...
   if (bind && bind->joykey != NO_BTN)
      input_config_get_bind_string_joykey(input_descriptor_label_show, buf, "", bind, size);
   else if (bind && bind->joyaxis != AXIS_NONE)
      input_config_get_bind_string_joyaxis(input_descriptor_label_show, buf, "", bind, size);
   else if (auto_bind && auto_bind->joykey != NO_BTN)
      input_config_get_bind_string_joykey(input_descriptor_label_show, buf, "Auto: ", auto_bind,
                                          size);
   else if (auto_bind && auto_bind->joyaxis != AXIS_NONE)
      input_config_get_bind_string_joyaxis(input_descriptor_label_show, buf, "Auto: ", auto_bind,
                                           size);
...
#ifndef RARCH_CONSOLE
   {
      ...
      if (key[0] == 'n' && key[1] == 'u' && key[2] == 'l' && key[3] == '\0')
         *key = '\0';
      /*empty?*/
      else if (*key != '\0')
      {
...
void input_config_get_bind_string_joykey(bool input_descriptor_label_show, char *buf,
                                         const char *prefix, const struct retro_keybind *bind,
                                         size_t size)
{
   if (GET_HAT_DIR(bind->joykey))
   {
      if (bind->joykey_label && !string_is_empty(bind->joykey_label) && input_descriptor_label_show)
      {
         size_t len = fill_pathname_join_delim(buf, prefix, bind->joykey_label, ' ', size);
         buf[len] = ' ';
         buf[len + 1] = '(';
         buf[len + 2] = 'h';
         buf[len + 3] = 'a';
      ...
      {
         const char *na_str = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_NOT_AVAILABLE);
         size_t len = snprintf(buf, size, "%sHat #%u ", prefix, (unsigned) GET_HAT(bind->joykey));
...
void input_config_get_bind_string_joyaxis(bool input_descriptor_label_show, char *buf,
                                          const char *prefix, const struct retro_keybind *bind,
                                          size_t size)
{
   if (bind->joyaxis_label && !string_is_empty(bind->joyaxis_label) && input_descriptor_label_show)
   {
      size_t len = fill_pathname_join_delim(buf, prefix, bind->joyaxis_label, ' ', size);
      buf[len] = ' ';
      buf[len + 1] = '(';
      buf[len + 2] = 'a';
      buf[len + 3] = 'x';
...
void osk_update_last_codepoint(unsigned *last_codepoint, unsigned *last_codepoint_len,
                               const char *word)
{
...
static unsigned get_kr_composition(char *pcur, char *padd)
{
   ...
   static char cc3[] = {"ㄱㄱㄲ ㄱㅅㄳ ㄴㅈㄵ ㄴㅎㄶ ㄹㄱㄺ ㄹㅁㄻ ㄹㅂㄼ ㄹㅅㄽ ㄹㅌㄾ ㄹㅍㄿ "
                        "ㄹㅎㅀ ㅂㅅㅄ ㅅㅅㅆ"};
   static char s1[] = {"ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎㅏㅐㅑㅒㅓㅔㅕㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠ"
                       "ㅡㅢㅣㆍㄱㄲㄳㄴㄵㄶㄷㄹㄺㄻㄼㄽㄾㄿㅀㅁㅂㅄㅅㅆㅇㅈㅊㅋㅌㅍㅎ"};
   char *tmp1 = NULL;
   char *tmp2 = NULL;
   int c1 = -1;
   int c2 = -1;
   ...
   else if (c3 > 0)
   {
      ...
      if (nv < 19)
      {
         ...
         strlcat(utf8, padd, sizeof(utf8));
         if (!(tmp2 = strstr(cc3, utf8)) || (tmp2 >= cc3 + sizeof(cc3) - 10))
            return ret;
         strlcpy(utf8, tmp2 + 6, 4);
         utf8[3] = 0;
 ...
 **/
static bool input_keyboard_line_event(input_driver_state_t *input_st, input_keyboard_line_t *state,
                                      uint32_t character)
{
   char array[2];
   bool ret = false;
   ...
   if (word)
      osk_update_last_codepoint(&input_st->osk_last_codepoint, &input_st->osk_last_codepoint_len,
                                word);
...
void input_event_osk_append(input_keyboard_line_t *keyboard_line, enum osk_type *osk_idx,
                            unsigned *osk_last_codepoint, unsigned *osk_last_codepoint_len, int ptr,
                            bool show_symbol_pages, const char *word, size_t word_len)
{
#ifdef HAVE_LANGEXTRA
   ...
   else if (string_is_equal(word, "\xe2\x87\xa7")) /* up arrow */
   ...
   else
   {
      ...
      osk_update_last_codepoint(osk_last_codepoint, osk_last_codepoint_len, word);
   }
}
...
bool input_driver_find_driver(settings_t *settings, const char *prefix, bool verbosity_enabled)
{
   int i = (int) driver_find_index("input_driver", settings->arrays.input_driver);

   if (i >= 0)
   {
      input_driver_st.current_driver = (input_driver_t *) input_drivers[i];
      RARCH_LOG("[Input]: Found %s: \"%s\".\n", prefix, input_driver_st.current_driver->ident);
   }
   else
   {
      ...
      {
         unsigned d;
         RARCH_ERR("Couldn't find any %s named \"%s\"\n", prefix, settings->arrays.input_driver);
 ...
 **/
bool input_set_sensor_state(unsigned port, enum retro_sensor_action action, unsigned rate)
{
   settings_t *settings = config_get_ptr();
   bool input_sensors_enable = settings->bools.input_sensors_enable;
   return input_driver_set_sensor(port, input_sensors_enable, action, rate);
}
 ...
 **/
bool input_set_rumble_state(unsigned port, enum retro_rumble_effect effect, uint16_t strength)
{
   settings_t *settings = config_get_ptr();
   ...
   return input_driver_set_rumble(port, joy_idx, effect, scaled_strength);
}
...
{
   settings_t *settings = config_get_ptr();
   if (input_driver_set_rumble_gain(gain, settings->uints.input_max_users))
      return true;
   return false;
}
...
void input_driver_init_joypads(void)
{
   ...
   if (!input_driver_st.primary_joypad)
      input_driver_st.primary_joypad = input_joypad_init_driver(
         settings->arrays.input_joypad_driver, input_driver_st.current_data);
#ifdef HAVE_MFI
   if (!input_driver_st.secondary_joypad)
      input_driver_st.secondary_joypad =
         input_joypad_init_driver("mfi", input_driver_st.current_data);
#endif
}
...
bool input_key_pressed(int key, bool keyboard_pressed)
{
   ...
   if (!((key < RARCH_BIND_LIST_END) && keyboard_pressed))
   {
      settings_t *settings = config_get_ptr();
      const input_device_driver_t *joypad =
         (const input_device_driver_t *) input_driver_st.primary_joypad;
...
bool video_driver_init_input(input_driver_t *tmp, settings_t *settings, bool verbosity_enabled)
{
   void *new_data = NULL;
   ...
   if (!input_driver_st.current_driver ||
       !(new_data = input_driver_init_wrap(input_driver_st.current_driver,
...
bool input_driver_grab_mouse(void)
{
   if (!input_driver_st.current_driver || !input_driver_st.current_driver->grab_mouse)
      return false;
   input_driver_st.current_driver->grab_mouse(input_driver_st.current_data, true);
   return true;
}
...
void input_config_reset(void)
{
   ...
   for (i = 1; i < MAX_USERS; i++)
      memcpy(input_config_binds[i], retro_keybinds_rest, sizeof(retro_keybinds_rest));
...
void input_config_set_device(unsigned port, unsigned id)
{
   ...
   if (settings && (port < MAX_USERS))
      configuration_set_uint(settings, settings->uints.input_libretro_device[port], id);
}
...
static void input_config_reindex_device_names(input_driver_state_t *input_st)
{
   ...
   for (i = 0; i < MAX_INPUT_DEVICES; i++)
   {
       ...
       * to the next device */
      if (string_is_empty(device_name) || input_st->input_device_info[i].name_index != 0)
         continue;
...
void input_config_set_device_config_path(unsigned port, const char *path)
{
   if (!string_is_empty(path))
   {
      ...
      input_driver_state_t *input_st = &input_driver_st;
      if (fill_pathname_parent_dir_name(parent_dir_name, path, sizeof(parent_dir_name)))
         fill_pathname_join_special(input_st->input_device_info[port].config_path, parent_dir_name,
...
#ifdef HAVE_COMMAND
void input_driver_init_command(input_driver_state_t *input_st, settings_t *settings)
{
   ...
   {
      input_driver_state_t *input_st = &input_driver_st;
      bool grab_stdin = input_st->current_driver->grab_stdin &&
...
void input_driver_deinit_command(input_driver_state_t *input_st)
{
   ...
   for (i = 0; i < ARRAY_SIZE(input_st->command); i++)
   {
      if (input_st->command[i])
         input_st->command[i]->destroy(input_st->command[i]);
...
void input_overlay_set_visibility(int overlay_idx, enum overlay_visibility vis)
{
   ...
   if (!input_st->overlay_visibility)
   {
      ...
      input_st->overlay_visibility =
         (enum overlay_visibility *) calloc(MAX_VISIBILITY, sizeof(enum overlay_visibility));
...
static bool video_driver_overlay_interface(const video_overlay_interface_t **iface)
{
...
static void input_overlay_loaded(retro_task_t *task, void *task_data, void *user_data,
                                 const char *err)
{
   size_t i;
   ...
   if (data->flags & OVERLAY_LOADER_ENABLE)
   {
      ...
      if ((data->flags & OVERLAY_LOADER_HIDE_IN_MENU) && (menu_st->flags & MENU_ST_FLAG_ALIVE))
         goto abort_load;
#endif
   ...
   if (!(data->flags & OVERLAY_LOADER_ENABLE) || !video_driver_overlay_interface(&iface) || !iface)
   {
   ...
   input_overlay_load_active(input_st->overlay_visibility, ol, data->overlay_opacity);
   ...
   input_overlay_set_scale_factor(ol, &data->layout_desc, video_st->width, video_st->height);
   ...
   if (inp_overlay_auto_rotate)
      input_overlay_auto_rotate_(video_st->width, video_st->height, input_overlay_enable,
...
void input_overlay_init(void)
{
   ...
   if (load_enabled)
   {
      ...
      task_push_overlay_load_default(input_overlay_loaded, path_overlay, overlay_hide_in_menu,
                                     overlay_hide_when_gamepad_connected, input_overlay_enable,
                                     overlay_opacity, &layout_desc, NULL);
   }
}
#endif
...
void input_pad_connect(unsigned port, input_device_driver_t *driver)
{
   ...
   input_autoconfigure_connect(driver->name(port), NULL, driver->ident, port, 0, 0);
}

static bool input_keys_pressed_other_sources(input_driver_state_t *input_st, unsigned i,
                                             input_bits_t *p_new_state)
{
#ifdef HAVE_COMMAND
   int j;
   for (j = 0; j < ARRAY_SIZE(input_st->command); j++)
      if ((i < RARCH_BIND_LIST_END) && input_st->command[j] && input_st->command[j]->state[i])
         return true;
#endif

#ifdef HAVE_OVERLAY
   if (input_st->overlay_ptr && ((BIT256_GET(input_st->overlay_ptr->overlay_state.buttons, i))))
      return true;
#endif

#ifdef HAVE_NETWORKGAMEPAD
   /* Only process key presses related to game input if using Remote RetroPad */
   if (i < RARCH_CUSTOM_BIND_LIST_END && input_st->remote &&
 ...
 */
static void input_keys_pressed(unsigned port, bool is_menu, unsigned input_hotkey_block_delay,
                               input_bits_t *p_new_state, const retro_keybind_set *binds,
                               ...
                               rarch_joypad_info_t *joypad_info)
{
   ...
   bool libretro_hotkey_set = binds[port][RARCH_ENABLE_HOTKEY].joykey != NO_BTN ||
   ...
   bool keyboard_hotkey_set = binds[port][RARCH_ENABLE_HOTKEY].key != RETROK_UNKNOWN;
   ...
   {
      if (input_state_wrap(input_st->current_driver, input_st->current_data,
                           input_st->primary_joypad, sec_joypad, joypad_info, &binds[port],
                           input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, port, RETRO_DEVICE_JOYPAD,
                           0, RARCH_ENABLE_HOTKEY))
      {
   ...
   if (!is_menu && binds[port][RARCH_GAME_FOCUS_TOGGLE].valid)
   {
      ...
      const struct retro_keybind *focus_normal = &binds[port][RARCH_GAME_FOCUS_TOGGLE];
      ...
      if (CHECK_INPUT_DRIVER_BLOCK_HOTKEY(focus_normal, focus_binds_auto))
      {
         if (input_state_wrap(input_st->current_driver, input_st->current_data,
                              input_st->primary_joypad, sec_joypad, joypad_info, &binds[port],
                              input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, port,
...

   {
      ...
      if (!(input_st->flags & INP_FLAG_BLOCK_LIBRETRO_INPUT) &&
         ...
         ret = input_state_wrap(input_st->current_driver, input_st->current_data,
                                input_st->primary_joypad, sec_joypad, joypad_info, &binds[port],
                                input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, port,
      ...
      if (!libretro_input_pressed)
      {
         ...
         keyboard_menu_pressed =
            binds[port][i].valid &&
            input_state_wrap(input_st->current_driver, input_st->current_data,
                             input_st->primary_joypad, sec_joypad, joypad_info, &binds[port],
                             input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, port,
                             RETRO_DEVICE_KEYBOARD, 0, input_config_binds[port][i].key);
         ...
         if (!keyboard_menu_pressed)
         {
            ...
            if (bit_pressed || BIT64_GET(lifecycle_state, i) ||
                input_keys_pressed_other_sources(input_st, i, p_new_state))
            {
               BIT256_SET_PTR(p_new_state, i);
            }
   ...
   if (input_st->flags & INP_FLAG_BLOCK_HOTKEY && (libretro_hotkey_set && keyboard_hotkey_set))
   {
            ...
            (!libretro_hotkey_set || !keyboard_hotkey_set))
   {
      ...
      for (i = RARCH_FIRST_META_KEY; i < RARCH_BIND_LIST_END; i++)
      {
         ...
         {
            if (binds[port][i].joykey != NO_BTN || binds[port][i].joyaxis != AXIS_NONE)
            {
...

   {
      for (i = RARCH_FIRST_META_KEY; i < RARCH_BIND_LIST_END; i++)
      {
         ...
         if (bit_pressed || other_pressed || BIT64_GET(lifecycle_state, i))
         {
...
static int16_t input_state_device(input_driver_state_t *input_st, settings_t *settings,
                                  input_mapper_t *handle, unsigned input_analog_dpad_mode,
                                  int16_t ret, unsigned port, unsigned device, unsigned idx,
                                  unsigned id, bool button_mask)
{
   int16_t res = 0;
   ...
   switch (device)
   {
      case RETRO_DEVICE_JOYPAD:
         ...
         if (id < RARCH_FIRST_META_KEY)
         {
...
#endif
            {
               ...
               if ((port == 0) && input_st->overlay_ptr &&
                   (input_st->overlay_ptr->flags & INPUT_OVERLAY_ALIVE) &&
                   BIT256_GET(input_st->overlay_ptr->overlay_state.buttons, id))
               {
#ifdef HAVE_MENU
                  bool menu_driver_alive = menu_state_get_ptr()->flags & MENU_ST_FLAG_ALIVE;
#else
                   ...
                   * is ignored */
                  if ((menu_driver_alive || !input_remap_binds_enable) || (id == remap_button))
                     res |= 1;
               }
#endif
            }
            ...
            if ((id < RETRO_DEVICE_ID_JOYPAD_UP) ||
                ((id > RETRO_DEVICE_ID_JOYPAD_RIGHT) && (id <= RETRO_DEVICE_ID_JOYPAD_R3)))
            {
               /*
               ...
               if (turbo_mode > INPUT_TURBO_MODE_CLASSIC)
               {
                  ...
                  else if (input_st->turbo_btns.turbo_pressed[port] >= 0)
                  {
                         ...
                         (1 << settings->uints.input_turbo_default_button))
                     {
                        ...
                        input_st->turbo_btns.enable[port] =
                           1 << button_map[MIN(ARRAY_SIZE(button_map) - 1,
                      ...
                      input_st->turbo_btns.enable[port] & (1 << id))
                  {
                     ...
                     res = ((input_st->turbo_btns.count % settings->uints.input_turbo_period) <
      ...
      case RETRO_DEVICE_KEYBOARD:
         ...
         if (id < RETROK_LAST)
         {
            ...
            if (port == 0)
            {
               if (input_st->overlay_ptr && (input_st->overlay_ptr->flags & INPUT_OVERLAY_ALIVE))
               {
                  input_overlay_state_t *ol_state = &input_st->overlay_ptr->overlay_state;
      ...
      case RETRO_DEVICE_ANALOG:
      {
         ...
         if (id == RETRO_DEVICE_ID_ANALOG_Y)
            base += 1;
#ifdef HAVE_NETWORKGAMEPAD
         if (input_st->remote && input_state && input_state->analog[base][port])
                  ...
                  else
                  {
...
#ifdef HAVE_OVERLAY
                     if (input_st->overlay_ptr &&
                         (input_st->overlay_ptr->flags & INPUT_OVERLAY_ALIVE) && (port == 0) &&
                            ...
                            (idx == RETRO_DEVICE_INDEX_ANALOG_RIGHT))))
                     {
                        ...
                        if (ol_analog)
                        {
                           ...
                           {
                              int16_t ol_analog_abs = (ol_analog >= 0) ? ol_analog : -ol_analog;
                              int16_t res_abs = (res >= 0) ? res : -res;

                              res = (ol_analog_abs > res_abs) ? ol_analog : res;
                           }
                        }
                     }
#endif
...
static void bsv_movie_handle_clear_key_events(bsv_movie_t *movie) { movie->key_event_count = 0; }

void bsv_movie_handle_push_key_event(bsv_movie_t *movie, uint8_t down, uint16_t mod, uint32_t code,
                                     uint32_t character)
{
   bsv_key_data_t data;
   data.down = down;
...
void bsv_movie_next_frame(input_driver_state_t *input_st)
{
   ...
   if (input_st->bsv_movie_state.flags & BSV_FLAG_MOVIE_RECORDING)
   {
      ...
      if (checkpoint_interval != 0 && handle->frame_ptr > 0 &&
          (handle->frame_ptr % (checkpoint_interval * 60) == 0))
      {
   ...
   if (input_st->bsv_movie_state.flags & BSV_FLAG_MOVIE_PLAYBACK)
   {
      ...
      {
         RARCH_ERR(
            "[Replay] Keyboard replay reading next frame while some unused keys still in queue\n");
      }
      if (intfstream_read(handle->file, &(handle->key_event_count), 1) == 1)
      {
         ...
         for (i = 0; i < handle->key_event_count; i++)
         {
            if (intfstream_read(handle->file, &(handle->key_events[i]), sizeof(bsv_key_data_t)) !=
...

      {
         ...
         if (intfstream_read(handle->file, (uint8_t *) (&next_frame_type), sizeof(uint8_t)) !=
             sizeof(uint8_t))
         {
            /* Unnatural EOF */
...
bool replay_get_serialized_data(void *buffer)
{
   ...
   if (input_st->bsv_movie_state.flags & (BSV_FLAG_MOVIE_RECORDING | BSV_FLAG_MOVIE_PLAYBACK))
   {
      ...
      if (read_amt != file_end)
         RARCH_ERR(
            "[Replay] Failed to write correct number of replay bytes into state file: %d / %d\n",
            read_amt, file_end);
   }
   return true;
}
...
bool replay_set_serialized_data(void *buf)
{
   ...
   if (!buffer)
   {
      if (recording)
      {
         ...
         runloop_msg_queue_push(str, 1, 180, true, NULL, MESSAGE_QUEUE_ICON_DEFAULT,
      ...
      if (is_compatible)
      {
         ...
         */
         if (loaded_len > handle_idx)
         {
            ...
            intfstream_write(input_st->bsv_movie_state_handle->file, buffer + sizeof(int32_t),
                             loaded_len);
         }
         else
...
void input_driver_poll(void)
{
   ...
   for (i = 0; i < max_users; i++)
   {
      ...
      input_st->turbo_btns.frame_enable[i] =
         (*input_st->libretro_input_binds[i])[RARCH_TURBO_ENABLE].valid
            ? input_state_wrap(input_st->current_driver, input_st->current_data, joypad, sec_joypad,
                               &joypad_info[i], (*input_st->libretro_input_binds),
                               input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, (unsigned) i,
   ...
   if (input_st->overlay_ptr && (input_st->overlay_ptr->flags & INPUT_OVERLAY_ALIVE))
   {
      ...
      input_poll_overlay(input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, settings,
                         input_st->overlay_ptr, input_st->overlay_visibility, input_overlay_opacity,
                         input_analog_dpad_mode, settings->floats.input_axis_threshold);
   }
#endif
   ...
   if (!(menu_state_get_ptr()->flags & MENU_ST_FLAG_ALIVE))
      ...
      if (input_remap_binds_enable)
      {
         ...
         bool poll_overlay = (overlay_pointer && (overlay_pointer->flags & INPUT_OVERLAY_ALIVE));
#endif
               ...
               case RETRO_DEVICE_ANALOG:
                  ...
                  if (joypad)
                  {
                     ...
                     int16_t ret = input_state_wrap(
                        input_st->current_driver, input_st->current_data, input_st->primary_joypad,
                        sec_joypad, &joypad_info[i], (*input_st->libretro_input_binds),
                        input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, (unsigned) i,
                        RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_MASK);
                     ...
                     for (k = 0; k < RARCH_FIRST_CUSTOM_BIND; k++)
                     {
                        if (ret & (1 << k))
                        {
                           bool valid_bind = (*input_st->libretro_input_binds[i])[k].valid;

                           if (valid_bind)
                           {
                              int16_t val = input_joypad_analog_button(
                                 input_analog_deadzone, input_analog_sensitivity, joypad,
                                 &joypad_info[i], k, &(*input_st->libretro_input_binds[i])[k]);
                              if (val)
                     ...
                     for (k = 0; k < 2; k++)
                     {
                        ...
                        for (j = 0; j < 2; j++)
                        {
                           ...
                           int16_t val = input_joypad_analog_axis(
                              input_analog_dpad_mode, input_analog_deadzone,
                              input_analog_sensitivity, joypad, &joypad_info[i], k, j,
            ...
            switch (device)
            {
               ...
               case RETRO_DEVICE_KEYBOARD:
                  for (j = 0; j < RARCH_CUSTOM_BIND_LIST_END; j++)
                  {
                     ...
                     unsigned remap_key = settings->uints.input_keymapper_ids[i][j];
                     ...
                     if (j >= RARCH_FIRST_CUSTOM_BIND && j < RARCH_ANALOG_BIND_LIST_END)
                     {
                        int16_t current_axis_value =
                           p_new_state->analogs[j - RARCH_FIRST_CUSTOM_BIND];
                        current_button_value =
                           abs(current_axis_value) > settings->floats.input_axis_threshold * 32767;
                     }
                     else
                     {
                     ...
                     if (poll_overlay && i == 0)
                     {
                        input_overlay_state_t *ol_state =
                           overlay_pointer ? &overlay_pointer->overlay_state : NULL;
                        if (ol_state)
                           current_button_value |= BIT256_GET(ol_state->buttons, j);
                     }
#endif
                     /* Press */
                     if ((current_button_value == 1) && !MAPPER_GET_KEY(handle, remap_key))
                     {
                        handle->key_button[remap_key] = (unsigned) j;

                        MAPPER_SET_KEY(handle, remap_key);
                        input_keyboard_event(true, remap_key, 0, 0, RETRO_DEVICE_KEYBOARD);
                     }
                     /* Release */
                     else if ((current_button_value == 0) && MAPPER_GET_KEY(handle, remap_key))
                     {
                        if (handle->key_button[remap_key] != j)
                           continue;

                        input_keyboard_event(false, remap_key, 0, 0, RETRO_DEVICE_KEYBOARD);
                ...
                * original input is cleared in input_state */
...
#endif
                     remap_valid = (current_button_value == 1) && (j != remap_button) &&
                                   (remap_button != RARCH_UNMAPPED);

#ifdef HAVE_ACCESSIBILITY
                     /* gamepad override */
                     if ((i == 0) && input_st->gamepad_input_override & (1 << j))
                     {
                     ...
                     if (remap_valid)
                     {
                        ...
                        else
                        {
                           ...
                           handle->analog_value[i][remap_button - RARCH_FIRST_CUSTOM_BIND] =
                              (p_new_state->analog_buttons[j] ? p_new_state->analog_buttons[j]
                                                              : 32767) *
                              invert;
                        }
                     }
                  ...
                  for (j = 0; j < 8; j++)
                  {
                     ...
                     if ((abs(current_axis_value) > 0 && (k != remap_axis) &&
   ...
   for (i = 0; i < ARRAY_SIZE(input_st->command); i++)
   {
      if (input_st->command[i])
      {
         memset(input_st->command[i]->state, 0, sizeof(input_st->command[i]->state));

         input_st->command[i]->poll(input_st->command[i]);
      }
   }
#endif
   ...
   if (input_st->remote)
   {
      ...
      for (user = 0; user < max_users; user++)
      {
         if (settings->bools.network_remote_enable_user[user])
         {
            ...
            ret =
               recvfrom(input_st->remote->net_fd[user], (char *) &msg, sizeof(msg), 0, NULL, NULL);
   ...
   if (BSV_MOVIE_IS_PLAYBACK_ON())
   {
      ...
      if (*key_event && *key_event == runloop_st->frontend_key_event)
      {
         ...
         for (i = 0; i < input_st->bsv_movie_state_handle->key_event_count; i++)
         {
            ...
            input_keyboard_event(k.down, swap_if_big32(k.code), swap_if_big32(k.character),
                                 swap_if_big16(k.mod), RETRO_DEVICE_KEYBOARD);
         }
...
int16_t input_state_internal(unsigned port, unsigned device, unsigned idx, unsigned id)
{
   rarch_joypad_info_t joypad_info;
   ...
   bitmask_enabled = (device == RETRO_DEVICE_JOYPAD) && (id == RETRO_DEVICE_ID_JOYPAD_MASK);
   ...
   while ((mapped_port = *(input_remap_port_map++)) < MAX_USERS)
   {
      ...
      ret = input_state_wrap(input_st->current_driver, input_st->current_data, joypad, sec_joypad,
                             &joypad_info, (*input_st->libretro_input_binds),
                             input_st->flags & INP_FLAG_KB_MAPPING_BLOCKED, mapped_port, device,
                             idx, id);

      if ((device == RETRO_DEVICE_ANALOG) && (ret == 0))
      {
         if (input_st->libretro_input_binds[mapped_port])
         {
                  ...
                  if (valid_bind)
                  {
                     if (sec_joypad)
                        ret = input_joypad_analog_button(
                           input_analog_deadzone, input_analog_sensitivity, sec_joypad,
                           &joypad_info, id, &(*input_st->libretro_input_binds[mapped_port])[id]);

                     if (joypad && (ret == 0))
                        ret = input_joypad_analog_button(
                           input_analog_deadzone, input_analog_sensitivity, joypad, &joypad_info,
                           id, &(*input_st->libretro_input_binds[mapped_port])[id]);
                  }
               }
            }
            else
            {
               if (sec_joypad)
                  ret = input_joypad_analog_axis(input_analog_dpad_mode, input_analog_deadzone,
                                                 input_analog_sensitivity, sec_joypad, &joypad_info,
                                                 idx, id,
                                                 (*input_st->libretro_input_binds[mapped_port]));

               if (joypad && (ret == 0))
                  ret = input_joypad_analog_axis(
                     input_analog_dpad_mode, input_analog_deadzone, input_analog_sensitivity,
                     joypad, &joypad_info, idx, id, (*input_st->libretro_input_binds[mapped_port]));
            }
         }
      }
      ...
      if (!input_blocked)
      {
         ...
         if (bitmask_enabled)
         {
            ...
            for (i = 0; i < RARCH_FIRST_CUSTOM_BIND; i++)
               if (input_state_device(input_st, settings, handle, input_analog_dpad_mode, ret,
                                      mapped_port, device, idx, i, true))
                  port_result |= (1 << i);
         }
         else
            port_result = input_state_device(input_st, settings, handle, input_analog_dpad_mode,
      ...
      if (device == RETRO_DEVICE_ANALOG)
      {
         ...
         {
            int16_t port_result_abs = (port_result >= 0) ? port_result : -port_result;
...
int16_t input_driver_state_wrapper(unsigned port, unsigned device, unsigned idx, unsigned id)
{
   input_driver_state_t *input_st = &input_driver_st;
   int16_t result = 0;
#ifdef HAVE_BSV_MOVIE
   ...
   if (BSV_MOVIE_IS_PLAYBACK_ON())
   {
      ...
      if (intfstream_read(input_st->bsv_movie_state_handle->file, &bsv_result, 2) == 2)
      {
#ifdef HAVE_CHEEVOS
         rcheevos_pause_hardcore();
#endif
   ...
   if ((device == RETRO_DEVICE_ANALOG) &&
       ((idx == RETRO_DEVICE_INDEX_ANALOG_LEFT) || (idx == RETRO_DEVICE_INDEX_ANALOG_RIGHT)))
...
#ifdef HAVE_HID
void *hid_driver_get_data(void) { return (void *) input_driver_st.hid_data; }
...
void input_remapping_cache_global_config(void)
{
   ...
   input_st->flags |= INP_FLAG_OLD_ANALOG_DPAD_MODE_SET | INP_FLAG_OLD_LIBRETRO_DEVICE_SET;
}
...
void input_remapping_restore_global_config(bool clear_cache)
{
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      if ((input_st->flags & INP_FLAG_OLD_ANALOG_DPAD_MODE_SET) &&
          (settings->uints.input_analog_dpad_mode[i] != input_st->old_analog_dpad_mode[i]))
         configuration_set_uint(settings, settings->uints.input_analog_dpad_mode[i],
                                input_st->old_analog_dpad_mode[i]);

      if ((input_st->flags & INP_FLAG_OLD_LIBRETRO_DEVICE_SET) &&
          (settings->uints.input_libretro_device[i] != input_st->old_libretro_device[i]))
         configuration_set_uint(settings, settings->uints.input_libretro_device[i],
...
void input_remapping_update_port_map(void)
{
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      ...
      if (remap_port < MAX_USERS)
      {
         ...
         settings->uints.input_remap_port_map[remap_port][port_map_index[remap_port]] = i;
...
void input_remapping_deinit(bool save_remap)
{
   ...
   runloop_st->flags &= ~(RUNLOOP_FLAG_REMAPS_CORE_ACTIVE | RUNLOOP_FLAG_REMAPS_CONTENT_DIR_ACTIVE |
...
void input_remapping_set_defaults(bool clear_cache)
{
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      ...
      for (j = 0; j < RARCH_FIRST_CUSTOM_BIND; j++)
      {
         ...
         configuration_set_uint(settings, settings->uints.input_remap_ids[i][j],
                                keybind ? keybind->id : RARCH_UNMAPPED);

         configuration_set_uint(settings, settings->uints.input_keymapper_ids[i][j],
                                RETROK_UNKNOWN);
      }

      /* Analog stick remaps */
      for (j = RARCH_FIRST_CUSTOM_BIND; j < (RARCH_FIRST_CUSTOM_BIND + 8); j++)
         configuration_set_uint(settings, settings->uints.input_remap_ids[i][j], j);

      /* Controller port remaps */
      configuration_set_uint(settings, settings->uints.input_remap_ports[i], i);
   }
...
void input_driver_collect_system_input(input_driver_state_t *input_st, settings_t *settings,
                                       input_bits_t *current_bits)
{
   ...
   bool menu_input_active =
      menu_is_alive && !(settings->bools.menu_unified_controls && !display_kb);
#endif
   input_driver_t *current_input = input_st->current_driver;
   unsigned max_users = settings->uints.input_max_users;
   ...
   for (port = 0; port < (int) max_users; port++)
   {
      ...
      input_keys_pressed(port,
                         ...
                         false,
#endif
                         block_delay, loop_bits, (const retro_keybind_set *) input_config_binds,
                         binds_norm, binds_auto, joypad, sec_joypad, &joypad_info);
   ...
   if (menu_input_active)
   {
       ...
       * port 0 */
      if (!display_kb && current_input && current_input->input_state)
      {
         unsigned i;
         unsigned ids[][2] = {
         ...
         {
            if (current_input->input_state(input_st->current_data, joypad, sec_joypad, &joypad_info,
                          ...
                          unsigned device)
{
...
#ifdef HAVE_ACCESSIBILITY
   if (menu_input_dialog_get_display_kb() && down &&
       is_accessibility_enabled(accessibility_enable, access_st->enabled))
   {
      if (code != 303 && code != 0)
      {
         ...
         if (say_char)
         {
            ...
            *say_char = c;
            ...
            else
            {
               ...
               if (lut_name)
                  accessibility_speak_priority(accessibility_enable,
                                               accessibility_narrator_speech_speed, lut_name, 10);
               else if (character != 0)
                  accessibility_speak_priority(accessibility_enable,
                                               accessibility_narrator_speech_speed, say_char, 10);
            }
            free(say_char);
         }
      }
   ...
   if (input_st->flags & INP_FLAG_DEFERRED_WAIT_KEYS)
   {
      ...
      input_st->flags &= ~(INP_FLAG_KB_MAPPING_BLOCKED | INP_FLAG_DEFERRED_WAIT_KEYS);
   }
   ...
   else if (input_st->keyboard_line.enabled)
   {
...
#ifdef NATIVE_BUILD
      /* Filter out navigation and modifier keys that shouldn't produce text input.
       * On native PC builds, these keys are used for OSK navigation via joypad
       * mapping, but their keycodes (>= 128) would be converted to '?' in text
       * input. This is not needed on real hardware where buttons don't generate
       * keyboard events. */
      switch (code)
      {
         case RETROK_UP:
      ...
      switch (device)
      {
         case RETRO_DEVICE_POINTER:
            ...
            /* fall-through */
         default:
            if (!input_keyboard_line_event(input_st, &input_st->keyboard_line, character))
               return;
            break;
      }
   ...
   else
   {
      ...
      if (!input_st->game_focus_state.enabled && BIT512_GET(input_st->keyboard_mapping_bits, code))
      {
         settings_t *settings = config_get_ptr();
         unsigned max_users = settings->uints.input_max_users;
         unsigned j;
         ...
         for (j = 0; j < max_users; j++)
         {
            ...
            if (j == 0 && !block_key_event &&
                !(!hotkey_pressed && hotkey_code != RETROK_UNKNOWN && hotkey_code != code))
            {
            ...
            if (input_config_get_device(j) && !block_key_event)
            {
      ...
      if (*key_event)
      {
         if (*key_event == runloop_st->frontend_key_event)
         {
            ...
            if (BSV_MOVIE_IS_RECORDING())
            {
               bsv_movie_handle_push_key_event(input_st->bsv_movie_state_handle, down, mod, code,
                                               character);
            }
#endif
         }
